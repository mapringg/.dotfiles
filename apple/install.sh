...
# Apple-specific dotfiles (hidden files starting with .)
for file in ~/.dotfiles/apple/.*; do
    if [ -f "$file" ] && [ "$(basename "$file")" != "." ] && [ "$(basename "$file")" != ".." ]; then
        create_symlink "$file" ~/"$(basename "$file")"
    fi
done

# Apple-specific .config directory
if [ -d ~/.dotfiles/apple/.config ]; then
    link_directory_contents ~/.dotfiles/apple/.config ~/.config
fi
