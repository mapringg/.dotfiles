---
name: test
description: Detects the test framework, runs the test suite, and fixes any failures. Use when asked to run tests, fix failing tests, or check test status.
---
...
1. **Check for test configuration files** to identify the framework:
   ...
   | `Gemfile` + `test/` | Minitest | `bundle exec rails test` |
   | `.NET` projects | .NET | `dotnet test` |
