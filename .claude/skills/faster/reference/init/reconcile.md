...
2. **Find reconcilable inits**: List all `.md` files in `.claude/rules/`
   - For each file, check if a corresponding master init exists at `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
   - Only include files that have a matching master init

3. **If the init name is already known from context**:
   - Validate it exists in both locations, proceed to Phase 2
   - If not known: proceed to selection

4. **Present selection** to the user:
...
2. **Read master init file**: `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
...
3. **Present diff summary**:
   ...
   Master (~/.claude/skills/faster/reference/init/helpers/init-{name}.md):
     - [Y] lines
...
1. **Extract and update master**:
   - Read `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
...
3. **Write if confirmed**:
   - Update `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
   - Report success
...
2. **For each differing section**, ask the user:
...
5. **Write if confirmed**:
   - Update `.claude/rules/{name}.md`
   - Update `~/.claude/skills/faster/reference/init/helpers/init-{name}.md` (content between markers)
   - Report success
...
Reconciliation complete:
  ...
  {Strategy applied}:
    - ~/.claude/skills/faster/reference/init/helpers/init-{name}.md — [updated/unchanged]
