...
If the init name is not already known from context, ask the user which init to update (e.g., `react`, `laravel`, `vue`).
...
1. Read `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
...
1. **Update the master init file**: Write to `~/.claude/skills/faster/reference/init/helpers/init-{name}.md`
