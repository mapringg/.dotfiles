return {
  ...
  config = function(_, opts)
    ...
    require("which-key").add({
      ...
      { "<leader>as", ":AmpSend ", desc = "Send message", icon = "󱙺" },
      { "<leader>ab", "<cmd>AmpSendBuffer<cr>", desc = "Send buffer", icon = "󱙺" },
      { "<leader>ap", ":AmpPromptSelection<cr>", desc = "Add selection to prompt", icon = "󱙺", mode = "v" },
      { "<leader>ar", ":AmpPromptRef<cr>", desc = "Add file reference to prompt", icon = "󱙺", mode = "v" },
    })
  end,
}